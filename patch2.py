...
function cleanup() {
    ...
    if (G.isOwner) {
        ...
        if (G.lockFd !== null) {
            try { fs.closeSync(G.lockFd); } catch {}
...
G = {"sock_path": "", "http_port": 0, "internal_http_port": 0, "tcp_port": 0, "active_sockets": [], "cleaning_up": False, "proc": None, "data_dir": "", "srv": None, "dirty": False, "is_owner": False, "lock_fd": None}
...
def _cleanup(*_):
    ...
    if G.get("is_owner"):
        ...
        if G.get("lock_fd"):
            with suppress(Exception): os.close(G["lock_fd"])
