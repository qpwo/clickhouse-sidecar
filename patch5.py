...
export async function runDaemon(dataDir) {
    try {
        ...
        for (const sock of G.pendingSockets) {
            if (!sock.destroyed) {
                sock.write(JSON.stringify({ error: G.chError }) + '\\n');
                sock.end();
            }
        }
        G.pendingSockets.clear();
...
function acquireLock(sockPath) {
    return new Promise((resolve, reject) => {
        ...
        const lockDir = path.join(G.dataDir, 'daemon.lock.dir');
        const tryLock = () => {
            try {
                ...
                try {
                    const pidStr = fs.readFileSync(path.join(lockDir, 'pid'), 'utf8').trim();
                    if (!pidStr) throw new Error('Empty pid');
                    const pid = parseInt(pidStr, 10);
                    if (isNaN(pid)) throw new Error('Empty pid');
                    missingPidCount = 0;
                    ...
                    if (e.code === 'ENOENT' || e.message === 'Empty pid') {
                        missingPidCount++;
                        if (missingPidCount <= 10) return setTimeout(tryLock, 100);
                    }
                    try { fs.renameSync(lockDir, lockDir + '.trash.' + crypto.randomUUID()); } catch {}
...
function cleanup() {
    ...
    if (G.isOwner) {
        try {
            const pidStr = fs.readFileSync(path.join(G.dataDir, 'daemon.lock.dir', 'pid'), 'utf8');
            if (parseInt(pidStr, 10) === process.pid) {
                try { fs.unlinkSync(G.sockPath); } catch {}
                try { fs.rmSync(path.join(G.dataDir, 'daemon.lock.dir'), { recursive: true, force: true }); } catch {}
            }
        } catch {}
    }
    if (G.proc) {
...
function setupAtexit() {
    ...
    process.on('uncaughtException', err => { log(`Uncaught: ${err.stack}`); cleanup(); });
}
...
async function startClickhouse(binPath) {
    ...
    proxySrv.on('error', err => {
        log(`Proxy bind error: ${err.message}`);
        process.exit(1);
    });
...
def run_daemon(data_dir):
    try:
        ...
        import traceback
        _log(f"Daemon crashed: {traceback.format_exc()}")
        sys.exit(1)
...
def _acquire_lock():
    lock_dir = os.path.join(G["data_dir"], "daemon.lock.dir")
    missing_pid_count = 0
    import uuid
    while True:
        try:
            ...
            try:
                ...
                if not pid_str: raise ValueError("Empty pid")
                missing_pid_count = 0
                pid = int(pid_str)
                os.kill(pid, 0)
                ...
                if isinstance(e, (FileNotFoundError, ValueError)):
                    missing_pid_count += 1
                    if missing_pid_count <= 10:
                        time.sleep(0.1)
                        continue
                trash = f"{lock_dir}.trash.{uuid.uuid4().hex}"
                with suppress(OSError): os.rename(lock_dir, trash)
            time.sleep(0.1)
...
def _cleanup(*_):
    ...
    if G.get("is_owner"):
        try:
            with open(os.path.join(G["data_dir"], "daemon.lock.dir", "pid")) as f:
                current_pid = int(f.read().strip())
            if current_pid == os.getpid():
                with suppress(Exception): os.remove(G["sock_path"])
                import shutil
                with suppress(Exception): shutil.rmtree(os.path.join(G["data_dir"], "daemon.lock.dir"), ignore_errors=True)
